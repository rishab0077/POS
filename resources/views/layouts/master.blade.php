...
<head>
    ...
    <title>{{ config('app.name', 'Restaurant POS') }} - @yield('title', config('app.name', 'Restaurant POS'))</title>
        ...
        <aside
            ...
            <nav class="flex-grow px-4 py-4 space-y-2 overflow-y-auto">
                ...
                @if ($user->canUsePos())
                    ...
                    @if (app(App\Http\Service\RestaurantService::class)->isKitchenModuleEnabled())
                        <x-nav-link :href="route('kitchen.index')" :openInNewTab="true" :active="request()->routeIs('kitchen.index')">Kitchen View</x-nav-link>
                    @endif
                @endif

                @if ($user->canManageInventory())
                    <x-nav-link :href="route('inventory.dashboard')" :active="request()->routeIs('inventory.*')">Inventory</x-nav-link>
                @endif
