            ...
            <div class="m-2 p-2 bg-slate-100 rounded">
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.users.update', $user->id) }}">
                        ...
                        <div class="sm:col-span-6 pt-5">
                            <p class="block text-sm font-medium text-gray-700">Multi-factor authentication</p>
                            <p class="mt-1 text-sm {{ $user->hasMfaEnabled() ? 'text-green-700' : 'text-gray-600' }}">
                                {{ $user->hasMfaEnabled() ? 'Enabled' : ($user->requiresMfa() ? 'Required — enrollment pending' : 'Not enabled') }}
                            </p>
                        </div>
                    ...
                    @if ($user->hasMfaEnabled())
                        <form method="POST" action="{{ route('admin.users.reset-mfa', $user) }}"
                            onsubmit="return confirm('Reset MFA and revoke this user’s sessions?');" class="p-4">
                            @csrf
                            <button type="submit"
                                class="px-4 py-2 bg-red-700 hover:bg-red-800 rounded-lg text-white">Reset MFA</button>
                        </form>
                    @endif
                </div>
            </div>
