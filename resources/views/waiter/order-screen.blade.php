<x-waiter-layout>
    ...
    <div class="bg-gray-100 min-h-screen font-sans text-gray-700 " x-data="waiterOrderScreen({
        table: {{ Js::from($table) }},
        existingTableTotal: {{ Js::from($existingTableTotal) }},
        ...
        <header class="sticky top-0 z-30 bg-white shadow-md p-3 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
            <h1 class="text-xl font-bold text-gray-800">
                Table: <span class="text-blue-600">{{ $table->name }}</span>
            </h1>
            <div class="grid w-full grid-cols-3 gap-2 text-right sm:w-auto sm:gap-5">
                <div>
                    <p class="text-xs text-gray-600 sm:text-sm">Current table</p>
                    <p class="font-bold text-gray-800 sm:text-lg" x-text="formatCurrency(existingTableTotal)"></p>
                </div>
                <div>
                    <p class="text-xs text-gray-600 sm:text-sm">New items</p>
                    <p class="font-bold text-blue-600 sm:text-lg" x-text="formatCurrency(subtotal)"></p>
                </div>
                <div>
                    <p class="text-xs text-gray-600 sm:text-sm">After adding</p>
                    <p class="font-bold text-green-600 sm:text-lg" x-text="formatCurrency(projectedTableTotal)"></p>
                </div>
            </div>
        </header>

        <main class="p-4 pb-48 sm:pb-32">
            <div class="space-y-3">
        ...
        <footer
            class="fixed bottom-0 left-0 w-full bg-white border-t-2 border-blue-600 p-3 z-30 flex flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-between md:left-64 md:w-[calc(100%-16rem)]">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center space-x-3">
                                    <button @click="updateQuantity(item.id, -1)"
                                        class="w-8 h-8 bg-red-500 text-white rounded-full font-bold text-lg leading-tight shadow-sm hover:bg-red-600">-</button>
                                    <span class="w-8 text-center text-lg font-semibold" x-text="item.quantity"></span>
                                    <button @click="updateQuantity(item.id, 1)"
                                        class="w-8 h-8 bg-green-500 text-white rounded-full font-bold text-lg leading-tight shadow-sm hover:bg-green-600">+</button>
                                </div>
                            </div>
    ...
    <script>
        function waiterOrderScreen(data) {
            return {
                table: data.table,
                existingTableTotal: Number(data.existingTableTotal) || 0,
                ...
                get projectedTableTotal() {
                    return this.existingTableTotal + this.subtotal;
                },
