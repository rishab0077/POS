<x-master-layout>
    ...
    <div class="container mx-auto p-4">
        ...
        <form method="post" action="{{ route('admin.restaurant.update.config') }}"
            ...
            <div class="mb-6">
                <label for="GST" class="block text-gray-700 text-sm font-bold mb-2">PAN/VAT:</label>
            ...
            <div class="mb-6">
                <label for="counter_printer" class="block text-gray-700 text-sm font-bold mb-2">Counter Printer:</label>
                <p class="text-xs text-red-400 font-semibold">Used for customer bills/card slips. Falls back to biller printer.</p>
                <input type="text" id="counter_printer" name="counter_printer"
                    value="{{ $restaurantConfig['counter_printer'] ?? '' }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            ...
            <div class="mb-6">
                <label for="bar_printer" class="block text-gray-700 text-sm font-bold mb-2">Bar Printer:</label>
                <p class="text-xs text-red-400 font-semibold">Used for BOT. Falls back to kitchen printer.</p>
                <input type="text" id="bar_printer" name="bar_printer"
                    value="{{ $restaurantConfig['bar_printer'] ?? '' }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
        ...
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="mb-4">
                ...
                {{-- Pusher Configuration Status - secrets must never be rendered into HTML --}}
                <div id="kitchenPusherSettings" class="mt-4 p-4 border rounded-md bg-gray-50 hidden">
                    <h3 class="text-lg font-semibold text-gray-700 mb-3">Kitchen Real-Time Service</h3>
                    <p class="text-sm text-gray-700">
                        Configuration is managed on the server. Credentials are intentionally hidden from the browser.
                    </p>
                </div>
            </div>
        </div>
            ...
            $('#toggleWaiterModuleBtn').on('click', function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        if (xhr.status === 423 && xhr.responseJSON && xhr.responseJSON.step_up_url) {
                            window.location.href = xhr.responseJSON.step_up_url;
                            return;
                        }
